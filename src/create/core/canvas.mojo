...
struct Canvas(Movable):
    ...
    def rect(mut self, x: Int, y: Int, w: Int, h: Int) raises:
        ...
        if self._fill_enabled:
            var c = self._fill
            for row in range(max(y0, 0), min(y0 + h, H)):
                for col in range(max(x0, 0), min(x0 + w, W)):
                    var off = (row * W + col) * 4
        ...
        if self._stroke_enabled:
            ...
            for row in range(max(y0 + h - sw, 0), min(y0 + h, H)):
                for col in range(max(x0, 0), min(x0 + w, W)):
                    var off = (row * W + col) * 4
                    px[unsafe_offset=off] = c.r; px[unsafe_offset=off + 1] = c.g
                    px[unsafe_offset=off + 2] = c.b; px[unsafe_offset=off + 3] = c.a
            for row in range(max(y0 + sw, 0), min(y0 + h - sw, H)):
                for col in range(max(x0, 0), min(x0 + sw, W)):
                    var off = (row * W + col) * 4
                    px[unsafe_offset=off] = c.r; px[unsafe_offset=off + 1] = c.g
                    px[unsafe_offset=off + 2] = c.b; px[unsafe_offset=off + 3] = c.a
                for col in range(max(x0 + w - sw, 0), min(x0 + w, W)):
                    var off = (row * W + col) * 4
                    px[unsafe_offset=off] = c.r; px[unsafe_offset=off + 1] = c.g
                    px[unsafe_offset=off + 2] = c.b; px[unsafe_offset=off + 3] = c.a
